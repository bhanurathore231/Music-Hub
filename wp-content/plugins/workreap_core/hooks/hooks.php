...
if (!function_exists('workreap_ajax_login')) {
    ...
    function workreap_ajax_login() { 
        ...
        if ($user_array['user_login'] == '') {
			...
			$username_or_email = $user_array['user_login'];

			$fetch_user = get_user_by('login', $username_or_email); 

			if (!$fetch_user) {
				$fetch_user = get_user_by('email', $username_or_email);
			}

			if($fetch_user){
				$user_id = $fetch_user->ID;
				$_is_verified = get_user_meta($user_id, '_is_verified', true);
				if($_is_verified == 'no'){
					echo json_encode(array('type' => 'error', 'loggedin' => false, 'message' => esc_html__('Your account is not active. Please active your account first or contact to administrator.', 'workreap_core')));
					exit();
				}
			}
							...
							<div class="form-group wt-checkbox-wrap">
								...
								<?php if(!empty($remove_role) && $remove_role !== 'freelancers'){?>
									<span class="wt-radio">
										...
										<label for="wt-freelancer-single"><?php esc_html_e('Artists', 'workreap_core'); ?></label>
									</span>
								<?php }?>
								<?php if(!empty($remove_role) && $remove_role !== 'employers'){?>
									<span class="wt-radio">
										...
										<label for="wt-employer-single"><?php esc_html_e('Labels', 'workreap_core'); ?></label>
									</span>
								<?php }?>
							</div>
...
if( !function_exists( 'workreap_username_add_remove' ) ){
	...
	function workreap_username_add_remove($classes=''){
		...
		<div class="form-group <?php echo esc_attr($classes);?>">
			<input type="text" name="username" class="form-control" value=""  placeholder="<?php esc_attr_e('Artist / Band name', 'workreap_core'); ?>">
		</div>
		<?php 
	}
}
