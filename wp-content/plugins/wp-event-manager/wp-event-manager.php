...
function disable_plugin_auto_update( $update, $item ) {
    // Specify the plugin file name or plugin slug for which you want to disable auto-updates
    $disabled_plugins = array(
        'plugin-folder/plugin-file.php',
        'plugin-slug',
    );

    if ( in_array( $item->slug, $disabled_plugins ) ) {
        // Disable auto-updates for the specified plugin
        return false;
    }
...
function hwe_disable_plugin_auto_update( $update, $item ) {
...
add_filter( 'auto_update_plugin', 'hwe_disable_plugin_auto_update', 10, 2 );
