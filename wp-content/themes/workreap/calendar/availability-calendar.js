	...
	AvailabilityCalendar.prototype = {
		...
		__createTable: function () {
			...
			for (var i = 0; i < 6; ++i) {
				...
				$tr.append('<td class="hwedataTarget" data-date=""></td><td class="hwedataTarget" data-date=""></td><td class="hwedataTarget" data-date=""></td><td class="hwedataTarget" data-date=""></td><td class="hwedataTarget" data-date=""></td><td class="hwedataTarget" data-date=""></td><td class="hwedataTarget" data-date=""></td>');
			}
		...
		__addThisMonthDays: function (date, year, month, cellIndexes, dates) {
			...
			for (var i = 1; i <= numDays; ++i) {
				...
				this.$cells.eq(dates.length).attr('data-date', year+'-'+("0" + (month + 1)).slice(-2)+'-'+i);
			...
			this.bookedDates.forEach(function (date) {
				if (date.start <= lastDate && date.end >= firstDate) {
					...
					if (startIndex !== undefined) {
						self.$cells.eq(startIndex).addClass('unavailable').append('<div class="first">'+date.event_title+'</div>');
						++startIndex;
					}
					else {
					...
					if (endIndex !== undefined) {
						//self.$cells.eq(endIndex).addClass('unavailable').append('<div class="last">'+date.event_title+'</div>');
						--endIndex;
					}
					else {
					...
					self.$cells.slice(startIndex, endIndex + 1).addClass('unavailable').append('<div>'+date.event_title+'</div>');
				}
				counter++;
			});
		},
