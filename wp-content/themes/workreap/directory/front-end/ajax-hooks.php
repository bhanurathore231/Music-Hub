...
if( !function_exists( 'workreap_update_freelancer_profile' ) ){
    function workreap_update_freelancer_profile(){ 
		...
		$facebook_followers=!empty( $_POST['basics']['facebook_followers'] ) ? $_POST['basics']['facebook_followers'] : '';
		$instagram_followers=!empty( $_POST['basics']['instagram_followers'] ) ? $_POST['basics']['instagram_followers'] : '';
		$sound_cloud_followers=!empty( $_POST['basics']['sound_cloud_followers'] ) ? $_POST['basics']['sound_cloud_followers'] : '';
		$tik_tok_followers=!empty( $_POST['basics']['tik_tok_followers'] ) ? $_POST['basics']['tik_tok_followers'] : '';
		$spotify_followers=!empty( $_POST['basics']['spotify_followers'] ) ? $_POST['basics']['spotify_followers'] : '';
		$artist_type = !empty($_POST['artist_type']) ? $_POST['artist_type'] : '';
		$music_genre = !empty($_POST['music_genre']) ? $_POST['music_genre'] : '';
		$number_of_members = !empty($_POST['number_of_members']) ? $_POST['number_of_members'] : '';
		$age_range = !empty($_POST['age_range']) ? $_POST['age_range'] : '';
		$instruments_associated_with_act = !empty($_POST['instruments_associated_with_act']) ? $_POST['instruments_associated_with_act'] : '';
		$years_performing = !empty($_POST['years_performing']) ? $_POST['years_performing'] : '';
		$next_gig = !empty($_POST['next_gig']) ? $_POST['next_gig'] : '';
		$school = !empty($_POST['school']) ? $_POST['school'] : '';
		$city = !empty($_POST['city']) ? $_POST['city'] : '';
		$country = !empty($_POST['country']) ? $_POST['country'] : '';
		$representation = !empty($_POST['representation']) ? $_POST['representation'] : '';


		//update artist type value
		$artist_type = array($artist_type); 
		wp_set_post_terms($post_id, $artist_type, 'artist_type');

		//update music genre value
		$music_genre = array($music_genre); 
		wp_set_post_terms($post_id, $music_genre, 'music_genre');

		//update number of members value
		$number_of_members = array($number_of_members); 
		wp_set_post_terms($post_id, $number_of_members, 'number_of_members');

		//update age range value
		$age_range = array($age_range); 
		wp_set_post_terms($post_id, $age_range, 'age_range');

		//update instruments associated with act value
		$instruments_associated_with_act = array($instruments_associated_with_act); 
		wp_set_post_terms($post_id, $instruments_associated_with_act, 'instruments_associated_with_act');

		//update years performing value
		$years_performing = array($years_performing); 
		wp_set_post_terms($post_id, $years_performing, 'years_performing');

		//update next gig value
		$next_gig = array($next_gig); 
		wp_set_post_terms($post_id, $next_gig, 'next_gig');

		//update school value
		$school = array($school); 
		wp_set_post_terms($post_id, $school, 'school');
		...
		wp_set_post_terms($post_id, $representation, 'representation');
		...
		update_user_meta($user_id, 'music_genre', $music_genre);
		update_user_meta($user_id, 'age_range', $age_range);
		update_user_meta($user_id, 'instruments_associated_with_act', $instruments_associated_with_act);
		update_user_meta($user_id, 'years_performing', $years_performing);
		update_user_meta($user_id, 'next_gig', $next_gig);
		update_user_meta($user_id, 'school', $school);
		update_user_meta($user_id, 'city', $city);
		update_user_meta($user_id, 'country', $country);
		update_user_meta($user_id, 'representation', $representation);
		update_user_meta($user_id, 'facebook_followers', $facebook_followers);
		update_user_meta($user_id, 'instagram_followers', $instagram_followers);
		update_user_meta($user_id, 'sound_cloud_followers', $sound_cloud_followers);
		update_user_meta($user_id, 'tik_tok_followers', $tik_tok_followers);
		update_user_meta($user_id, 'spotify_followers', $spotify_followers);
...
if ( !function_exists( 'workreap_post_job' ) ) {
	...
	function workreap_post_job() {
		...
		if( $post_id ){
			...
			//Dinesh code
			$artist_type              = !empty( $_POST['job']['artist_type'] ) ? $_POST['job']['artist_type'] : array();
			$music_genre              = !empty( $_POST['job']['music_genre'] ) ? $_POST['job']['music_genre'] : array();
			$number_of_members        = !empty( $_POST['job']['number_of_members'] ) ? $_POST['job']['number_of_members'] : array();
			$instruments_associated_with_act = !empty( $_POST['job']['instruments_associated_with_act'] ) ? $_POST['job']['instruments_associated_with_act'] : array();
			$age_range                = !empty( $_POST['job']['age_range'] ) ? $_POST['job']['age_range'] : array();
			$years_performing         = !empty( $_POST['job']['years_performing'] ) ? $_POST['job']['years_performing'] : array();
			$school              = !empty( $_POST['job']['school'] ) ? $_POST['job']['school'] : array();
			$next_gig              = !empty( $_POST['job']['next_gig'] ) ? $_POST['job']['next_gig'] : array();
			$city              = !empty( $_POST['job']['city'] ) ? $_POST['job']['city'] : array();
			$country              = !empty( $_POST['job']['country'] ) ? $_POST['job']['country'] : array();
			$representation              = !empty( $_POST['job']['representation'] ) ? $_POST['job']['representation'] : array();
			...
			wp_set_post_terms( $post_id, $number_of_members, 'number_of_members' );
			//update instruments_associated_with_act
			wp_set_post_terms( $post_id, $instruments_associated_with_act, 'instruments_associated_with_act' );
			//update age_range
			wp_set_post_terms( $post_id, $age_range, 'age_range' );
			//update years_performing
			wp_set_post_terms( $post_id, $years_performing, 'years_performing' );
			//update school
			wp_set_post_terms( $post_id, $school, 'school' );
			//update next_gig
